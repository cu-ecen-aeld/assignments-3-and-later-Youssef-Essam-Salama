...
typedef long long loff_t; // For compatibility with kernel types
#endif
...
struct aesd_circular_buffer {
	...
	/**
	 * The current location in the entry structure where the next write
	 * should be stored.
	 */
	uint8_t in_offs;
	/**
...
extern size_t
aesd_circular_buffer_find_EOF_offset(struct aesd_circular_buffer *buffer);
