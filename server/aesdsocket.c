...
static uint8_t add_thread_to_list(pthread_t thread_id, int sock_fd, const char *client_ip);
static uint8_t create_client_communication_thread(int sock_fd, const char *client_ip);
...
static uint8_t setup_socket(void)
{
    ...
    {
        do {
            server_sock_fd = socket(addrinfo->ai_family, addrinfo->ai_socktype, addrinfo->ai_protocol);
            if (server_sock_fd == -1)
            {
            ...
            if (0 != setsockopt(server_sock_fd, SOL_SOCKET, SO_REUSEADDR, &socket_reuse_option, sizeof(socket_reuse_option)))
            {
            ...
            if (0 != bind(server_sock_fd, addrinfo->ai_addr, addrinfo->ai_addrlen))
            {
    ...
    if ((EXIT_SUCCESS == ret_val) && (0 != listen(server_sock_fd, SOCKET_BACKLOG)))
    {
...
static void try_joining_client_communication_threads(void)
{
    ...
    SLIST_FOREACH_SAFE(current_thread_node, &thread_list, next_thread_node, next_thread_node)
    {
        if (TRUE == current_thread_node->thread_completed)
        {
            pthread_join(current_thread_node->thread_id, NULL);
            SLIST_REMOVE(&thread_list, current_thread_node, thread_node, next_thread_node);
...
static void close_client_communication_threads_sockets(void)
{
    ...
    SLIST_FOREACH_SAFE(current_thread_node, &thread_list, next_thread_node, next_thread_node)
    {
        /* Force client communication thread break out of recv() system call */
...
static uint8_t send_file(int sock_fd, char *buffer, uint32_t buffer_size)
{
    ...
    else
    {
        do {
            ...
            {
                bytes_sent = send(sock_fd, buffer + total_bytes_sent, bytes_read - total_bytes_sent, 0);
                if (bytes_sent == -1)
                {
        ...
        if ((EXIT_SUCCESS == ret_val) && (0 != fseek(log_file, 0, SEEK_END)))
        {
...
static void* handle_client_communication(void* arg)
{
    ...
    {
        do {
            bytes_received = recv(sock_fd, buffer, SOCKET_BUFFER_SIZE, 0);
            if (bytes_received > 0)
            {
                for (uint16_t i = 0U; i < bytes_received; i++)
                {
                    ...
                    if (buffer[i] == '\n')
                    {
                        ...
                        fflush(log_file);
                        ret_val = send_file(sock_fd, packet, packet_size);
...
static uint8_t add_thread_to_list(pthread_t thread_id, int sock_fd, const char *client_ip)
{
    ...
    else
    {
        ...
        strncpy(thread_node->client_ip, client_ip, sizeof(thread_node->client_ip) - 1U);
        thread_node->client_ip[sizeof(thread_node->client_ip) - 1U] = '\0';
...
static uint8_t create_client_communication_thread(int sock_fd, const char *client_ip)
{
    ...
    do
    {
        ...
        ret_val = pthread_create(&thread_id, NULL, &handle_client_communication, (void *)thread_node);
        if (ret_val != 0)
        {
...
static uint8_t handle_client_connections(void)
{
    ...
    while (TRUE == process_running)
    {
        ...
        accepted_sock_fd = accept(server_sock_fd, &client_addr, &client_addr_len);
        ...
        ret_val = create_client_communication_thread(accepted_sock_fd, client_ip);
        if (EXIT_SUCCESS != ret_val)
        {
            break;
        }
